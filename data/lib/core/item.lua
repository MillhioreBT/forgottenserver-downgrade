function Item.getType(self) return ItemType(self:getId()) end

function Item.isContainer(self) return false end

function Item.isCreature(self) return false end

function Item.isMonster(self) return false end

function Item.isNpc(self) return false end

function Item.isPlayer(self) return false end

function Item.isTeleport(self) return false end

function Item.isPodium(self) return false end

function Item.isTile(self) return false end
...
do
	...
	function Item:getNameDescription(subType, addArticle)
		return internalItemGetNameDescription(self:getType(), self, subType,
		                                      addArticle)
	end
...
do
	...
	local housePriceVisible = configManager.getBoolean(
		                          configKeys.HOUSE_DOOR_SHOW_PRICE)
	local pricePerSQM = configManager.getNumber(configKeys.HOUSE_PRICE)

	local showAtkWeaponTypes = {
		WEAPON_CLUB, WEAPON_SWORD, WEAPON_AXE, WEAPON_DISTANCE
	}
	local showDefWeaponTypes = {
		WEAPON_CLUB, WEAPON_SWORD, WEAPON_AXE, WEAPON_DISTANCE, WEAPON_SHIELD
	}
	...
	local function internalItemGetDescription(item, lookDistance, subType,
	                                          addArticle)
		...
		do
			...
			if not isUnique then
				...
				if isContainer then
					descriptions[#descriptions + 1] = string.format("Vol:%d",
		...
		do
			...
			if itemType:isBow() then
				descriptions[#descriptions + 1] = string.format("Range:%d",
				                                                item:getShootRange())
			...
			elseif table.contains(showAtkWeaponTypes, weaponType) then
				...
				if elementDmg ~= 0 then
					atkString = string.format("%s physical %+d %s", atkString, elementDmg,
		...
		do
			...
			if atkSpeed ~= 0 then
				descriptions[#descriptions + 1] = string.format("AS:%0.2f/turn",
				                                                2000 / atkSpeed)
			end
		end
		...
		do
			...
			if showDef then
				...
				if #defAttrs > 0 then
					descriptions[#descriptions + 1] = string.format("Def:%s", table.concat(
						                                                defAttrs, " "))
				end
			end
		end
		...
		do
			...
			for _, statData in pairs(stats) do
				...
				if #displayValues > 0 then
					...
					descriptions[#descriptions + 1] = string.format("%s %s", statData.name,
		...
		do
			for skill, value in pairs(abilities.specialSkills) do
				if value ~= 0 then
					...
					descriptions[#descriptions + 1] = string.format("%s %s%%",
					                                                getSpecialSkillName(
						                                                skill - 1), value)
				end
			end
		...
		do
			...
			for element, value in pairs(abilities.absorbPercent) do
				if value ~= 0 then
					protections[#protections + 1] = string.format("%s %+d%%", getCombatName(
						                                              2 ^ (element - 1)), value)
				end
			end

			if #protections > 0 then
				descriptions[#descriptions + 1] = string.format("protection %s",
				                                                table.concat(protections,
				                                                             ", "))
			end
		end
		...
		-- regeneration
		if abilities.manaGain > 0 or abilities.healthGain > 0 or
			abilities.regeneration then
		...
		if abilities.speed ~= 0 then
			descriptions[#descriptions + 1] = string.format("speed %+d", math.floor(
				                                                abilities.speed / 2))
		end
		...
		-- fluid type
		do
			if (itemGroup == ITEM_GROUP_FLUID or itemGroup == ITEM_GROUP_SPLASH) and
				subType > 0 then
				local subTypeName = getSubTypeName(subType)
				response[#response + 1] = string.format(" of %s", (subTypeName ~= "" and
					                                        subTypeName or "unknown"))
			end
		end
			...
			if not isVirtual then
				...
				if minLevelDoor ~= 0 then
					if actionId >= minLevelDoor then
						response[#response + 1] = string.format(" for level %d",
						                                        actionId - minLevelDoor)
					end
				end
			end
		end
		...
		do
			...
			if itemType:hasShowCharges() then
				...
				expireInfo[#expireInfo + 1] = string.format("has %d charge%s left", charges,
				                                            (charges ~= 1 and "s" or ""))
			end
			...
			if itemType:hasShowDuration() then
				...
				if maxDuration == 0 then
					...
					if transferType ~= 0 then
						...
						maxDuration = transferType and transferType:getDuration() * 1000 or
							              maxDuration
					end
				end
			...
			if #expireInfo > 0 then
				response[#response + 1] = string.format(" that %s",
				                                        table.concat(expireInfo, " and "))
			end
		end
			...
			if tile then
				...
				if house then
					...
					response[#response + 1] = string.format(
						                          ...
						                          houseName, houseOwnerName)
					if housePriceVisible and isForSale and pricePerSQM > 0 then
						response[#response + 1] = string.format(" It costs %d gold coins.",
						                                        pricePerSQM * house:getTileCount())
					end
				end
			end
		end
			...
			if itemType:isRune() then
				...
				if rune then
					...
					if hasLevel or hasMLvl or hasVoc then
						...
						if #vocAttrs > 1 then
							vocAttrs[#vocAttrs - 1] =
								string.format("and %s", vocAttrs[#vocAttrs - 1])
						end
						...
						local levelStr = ""
						if #levelInfo > 0 then
							levelStr = string.format(" of %s or higher",
							                         table.concat(levelInfo, " and "))
						end
				...
				if wieldInfo ~= 0 then
					...
					if (wieldInfo & WIELDINFO_LEVEL) ~= 0 then
						levelInfo[#levelInfo + 1] = string.format("level %d",
						                                          itemType:getMinReqLevel())
					end

					if (wieldInfo & WIELDINFO_MAGLV) ~= 0 then
						levelInfo[#levelInfo + 1] = string.format("magic level %d",
						                                          itemType:getMinReqMagicLevel())
					end

					if #levelInfo > 0 then
						wieldAttrs[#wieldAttrs + 1] = string.format("of %s or higher",
						                                            table.concat(levelInfo,
						                                                         " and "))
					end
		...
		if lookDistance <= 1 then
			...
			if isPickupable and not isUnique then
				response[#response + 1] = string.format("\n%s %0.2f oz.", (count == 1 or
					                                        not itemType:hasShowCount()) and
					                                        "It weighs" or "They weigh",
				                                        weight / 100)
			end
		end
				...
				if lookDistance <= 4 then
					...
					if writer and writer:len() > 0 then
						...
						if writeDate and writeDate > 0 then
							writeInfo[#writeInfo + 1] = string.format(" on %s",
							                                          os.date("%d %b %Y", writeDate))
						end
					end

					if #writeInfo > 0 then
						response[#response + 1] = string.format("%s:\n%s",
						                                        table.concat(writeInfo, ""), text)
					else
						response[#response + 1] = string.format("\nYou read: %s", text)
					end
				else
		...
		if lookDistance <= 1 or (not isVirtual and (isDoor or isBed)) then
			...
			if isDoor and lookDistance <= 1 and (not desc or desc == "") and
				itemType:getLevelDoor() ~= 0 then desc = "Only the worthy may pass." end
