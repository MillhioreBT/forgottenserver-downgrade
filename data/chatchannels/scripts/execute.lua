...
function onSpeak(player, messageType, message)
	...
	environment.print = function (...)
		...
		return player:sendChannelMessage("Print:", table.concat(args, "\t"), TALKTYPE_CHANNEL_O, CHAT_ID)
	end
	...
	local error <const> = function (err)
		...
		player:sendChannelMessage("Error:", err:sub(last + 1), TALKTYPE_CHANNEL_R1, CHAT_ID)
		print(string.format("[Error - Lua Channel]: Author: %s, Error: %s", player:getName(), err))
	end
	...
	if not result then
		...
		if not result then
			...
			if results.n ~= 0 then
				player:sendChannelMessage("Result:", table.concat(results, "\t"), TALKTYPE_CHANNEL_O, CHAT_ID)
			end
		end
	end
