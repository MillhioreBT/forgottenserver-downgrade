...
template <class T>
static void getSpectators(lua_State* L, int32_t arg, SpectatorVec& spectators)
{
	if (LuaScriptInterface::isUserdata(L, arg)) {
		if (T* creature = LuaScriptInterface::getUserdata<T>(L, arg)) {
			spectators.emplace_back(creature);
		}
		return;
	} else if (!LuaScriptInterface::isTable(L, arg)) {
		return;
	}

	lua_pushnil(L);
	while (lua_next(L, arg) != 0) {
		if (LuaScriptInterface::isUserdata(L, -1)) {
			if (T* creature = LuaScriptInterface::getUserdata<T>(L, -1)) {
				spectators.emplace_back(creature);
			}
		} else if (LuaScriptInterface::isInteger(L, -1)) {
			Creature* creature = g_game.getCreatureByID(LuaScriptInterface::getInteger<uint32_t>(L, -1));
			if (dynamic_cast<T*>(creature) != nullptr) {
				spectators.emplace_back(creature);
			}
		}
		lua_pop(L, 1);
...
std::optional<uint8_t> LuaScriptInterface::getBlessingId(lua_State* L, int32_t arg)
{
...
void LuaScriptInterface::registerFunctions()
{
	...
	registerMetaMethod("NetworkMessage", "__close", LuaScriptInterface::luaNetworkMessageDelete);
	...
	registerMethod("Creature", "sendCreatureSquare", LuaScriptInterface::luaCreatureSendCreatureSquare);
	...
	registerMethod("Player", "isAccountManager", LuaScriptInterface::luaPlayerIsAccountManager);
	...
	registerMethod("Player", "setTibiaCoins", LuaScriptInterface::luaPlayerSetTibiaCoins);
	...
	registerMetaMethod("OfflinePlayer", "__close", LuaScriptInterface::luaOfflinePlayerRemove);
	...
	registerMetaMethod("Combat", "__close", LuaScriptInterface::luaCombatDelete);
	...
	registerMethod("Condition", "setInitDamage", LuaScriptInterface::luaConditionSetInitDamage);
	registerMethod("Condition", "getTotalDamage", LuaScriptInterface::luaConditionGetTotalDamage);
	...
	registerMetaMethod("Loot", "__close", LuaScriptInterface::luaDeleteLoot);
	...
	registerMetaMethod("MonsterSpell", "__close", LuaScriptInterface::luaDeleteMonsterSpell);
	...
	registerMetaMethod("XMLDocument", "__close", LuaScriptInterface::luaDeleteXmlDocument);
	...
	registerMetaMethod("XMLNode", "__close", LuaScriptInterface::luaDeleteXmlNode);
...
void LuaScriptInterface::registerVariable(std::string_view tableName, std::string_view name, lua_Integer value)
{
	...
	setField(luaState, name.data(), value);
...
{
	// transformToSHA1(text)
	pushString(L, transformToSHA1(getString(L, 1)));
	return 1;
}
...
int LuaScriptInterface::luaDoAreaCombat(lua_State* L)
{
	...
	if (!creature && (!isInteger(L, 1) || getInteger<uint32_t>(L, 1) != 0)) {
	...
	if (g_config[ConfigKeysBoolean::WARN_UNSAFE_SCRIPTS] || g_config[ConfigKeysBoolean::CONVERT_UNSAFE_SCRIPTS]) {
		...
		for (int i = 3; i <= parameters; ++i) {
			...
			switch (type) {
				...
				case LuaData_Monster:
				case LuaData_Npc: {
					if (auto creature = getCreature(L, i)) {
...
{
	// sendChannelMessage(channelId, type, message)
	uint16_t channelId = getInteger<uint16_t>(L, 1);
...
{
	pushString(L, g_config[getInteger<ConfigKeysString>(L, -1)]);
	return 1;
}

int LuaScriptInterface::luaConfigManagerGetNumber(lua_State* L)
{
	lua_pushinteger(L, g_config[getInteger<ConfigKeysInteger>(L, -1)]);
	return 1;
}

int LuaScriptInterface::luaConfigManagerGetBoolean(lua_State* L)
{
	pushBoolean(L, g_config[getInteger<ConfigKeysBoolean>(L, -1)]);
	return 1;
}
...
int LuaScriptInterface::luaCreatureSendCreatureSquare(lua_State* L)
{
...
int LuaScriptInterface::luaPlayerIsAccountManager(lua_State* L)
{
...
int LuaScriptInterface::luaPlayerSetTibiaCoins(lua_State* L)
{
...
int LuaScriptInterface::luaPlayerSetMana(lua_State* L)
{
	...
	auto mana = getInteger<uint32_t>(L, 2);
	player->mana = std::min<uint32_t>(mana, player->manaMax);
	player->sendStats();
	pushBoolean(L, true);
	return 1;
}
...
int LuaScriptInterface::luaPlayerSetMaxMana(lua_State* L)
{
	...
	Player* player = getPlayer(L, 1);
	if (player) {
		player->manaMax = getInteger<uint32_t>(L, 2);
...
int LuaScriptInterface::luaPlayerShowTextDialog(lua_State* L)
{
	...
	player->maxWriteLen = static_cast<uint16_t>(length);
...
int LuaScriptInterface::luaPlayerOpenContainer(lua_State* L)
{
...
int LuaScriptInterface::luaPlayerCloseContainer(lua_State* L)
{
...
int LuaScriptInterface::luaOfflinePlayerRemove(lua_State* L)
{
...
int LuaScriptInterface::luaCombatSetParameter(lua_State* L)
{
	...
	if (!combat) {
		reportErrorFunc(L, getErrorDesc(LuaErrorCode::COMBAT_NOT_FOUND));
		lua_pushnil(L);
		return 1;
	}
...
int LuaScriptInterface::luaCombatSetCallback(lua_State* L)
{
	...
	if (!combat->loadCallBack(key, getScriptEnv()->getScriptInterface())) {
		reportErrorFunc(L, getErrorDesc(LuaErrorCode::CALLBACK_NOT_FOUND));
		pushBoolean(L, false);
		return 1;
	}
...
int LuaScriptInterface::luaCombatExecute(lua_State* L)
{
	...
	switch (variant.type()) {
		...
		case VARIANT_NONE: {
			reportErrorFunc(L, getErrorDesc(LuaErrorCode::VARIANT_NOT_FOUND));
			pushBoolean(L, false);
			return 1;
		}
...
int LuaScriptInterface::luaConditionSetInitDamage(lua_State* L)
{
...
int LuaScriptInterface::luaConditionGetTotalDamage(lua_State* L)
{
...
{
	// monsterType:addSummon(name, speed, chance)
	MonsterType* monsterType = getUserdata<MonsterType>(L, 1);
	if (monsterType) {
...
void LuaEnvironment::executeTimerEvent(uint32_t eventIndex)
{
	...
	for (auto parameter : boost::adaptors::reverse(timerEventDesc.parameters)) {
		...
		auto type = getInteger<LuaDataType>(luaState, -1);
		lua_pop(luaState, 2);
		...
		switch (type) {
			...
			case LuaData_Npc: {
				if (lua_getiuservalue(luaState, -1, 1)) {
					auto creatureId = getInteger<uint32_t>(luaState, -1);
					if (auto creature = g_game.getCreatureByID(creatureId)) {
						pushUserdata<Creature>(luaState, creature);
